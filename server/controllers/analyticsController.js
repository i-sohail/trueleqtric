...
exports.getMonthlyTrend = asyncHandler(async (req, res) => {
  ...
  const [spos, leads] = await Promise.all([
    prisma.salesPO.findMany({
      where: {
        isDeleted: false,
        poDate: { gte: new Date(`${year}-01-01`), lte: new Date(`${year}-12-31`) }
      }
    }),
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    months[k] = { label: new Date(year, m - 1).toLocaleString('en', { month: 'short' }), rev: 0, margin: 0, leads: 0 };
  }
  spos.forEach(p => {
    if (!p.poDate) return;
    const k = p.poDate.toISOString().slice(0, 7);
    if (months[k]) {
      const c = calcSPO(p);
      months[k].rev += c.rev;
      months[k].margin += c.margin;
    }
  });
  leads.forEach(l => {
    if (!l.date) return;
    const k = new Date(l.date).toISOString().slice(0, 7);
    if (months[k]) months[k].leads++;
  });
...
exports.getCategoryRevenue = asyncHandler(async (req, res) => {
  ...
  spos.forEach(p => {
    ...
    const c = calcSPO(p);
    if (!cats[cat]) cats[cat] = { rev: 0, margin: 0, count: 0 };
    cats[cat].rev += c.rev;
    cats[cat].margin += c.margin;
    cats[cat].count++;
  });
...
exports.getSalesRepScoreboard = asyncHandler(async (req, res) => {
  const [leads, spos] = await Promise.all([
    prisma.lead.findMany({ where: { isDeleted: false } }),
    prisma.salesPO.findMany({ where: { isDeleted: false } })
  ]);
  const reps = {};
  leads.forEach(l => {
    const r = l.salesRep || 'Unassigned';
    if (!reps[r]) reps[r] = { leads: 0, won: 0, pipeline: 0, revenue: 0 };
    reps[r].leads++;
    if (l.stage === 'PO Received') reps[r].won++;
    reps[r].pipeline += (l.estValue || 0);
  });
  spos.forEach(p => {
    const r = p.salesRep || 'Unassigned';
    if (!reps[r]) reps[r] = { leads: 0, won: 0, pipeline: 0, revenue: 0 };
    reps[r].revenue += calcSPO(p).rev;
  });
...
exports.getForecast = asyncHandler(async (req, res) => {
  ...
  const spos = await prisma.salesPO.findMany({
    where: { isDeleted: false, poDate: { gte: new Date(`${year}-01-01`) } }
  });
  const monthly = {};
  for (let m = 1; m <= 12; m++) {
    const k = `${year}-${String(m).padStart(2, '0')}`;
    monthly[k] = { label: new Date(year, m - 1).toLocaleString('en', { month: 'short' }), rev: 0 };
  }
  spos.forEach(p => {
    if (!p.poDate) return;
    const k = p.poDate.toISOString().slice(0, 7);
    if (monthly[k]) monthly[k].rev += calcSPO(p).rev;
  });
  const revArr = Object.values(monthly).map(m => m.rev);
  const forecast = ['Oct', 'Nov', 'Dec'].map((label, i) => {
    const w = revArr.slice(Math.max(0, i + 5), i + 8);
    return { label, value: w.length ? w.reduce((s, v) => s + v, 0) / w.length : 0 };
  });
...
exports.getMarginWaterfall = asyncHandler(async (req, res) => {
  ...
  const data = spos.map(p => {
    const c = calcSPO(p);
    const freightCost = p.freightCost || 0;
    const ldDeduction = p.ldDeduction || 0;
    const actualMargin = c.margin - freightCost - ldDeduction;
    const leakage = c.margin - actualMargin;
