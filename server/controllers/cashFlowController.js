...
exports.getProjection = asyncHandler(async (req, res) => {
  const [arRecords, apRecords, schedules] = await Promise.all([
    prisma.aR.findMany({ where: { isDeleted: false, NOT: { status: { in: ['Received - Full','Written Off'] } } } }),
    prisma.aP.findMany({ where: { isDeleted: false, status: 'Pending' } }),
    prisma.paymentSchedule.findMany({ where: {} }),
  ]);
